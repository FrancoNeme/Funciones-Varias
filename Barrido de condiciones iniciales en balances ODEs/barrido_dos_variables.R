...
#___ Seteo de variable manipulada C_S0 


lim_inf_C_S0 = 5 # Cota inferior
lim_sup_C_S0 = 50 # Cota superior 
paso_C_S0 = 5 # Paso de iteracion

vector_C_S0 = seq(lim_inf_C_S0,lim_sup_C_S0, by = paso_C_S0) # Vector de valores de barrido de la variable manipulada C_S0. Estos son los valores que va a tener la variable manipulada



#___ Seteo de variable manipulada C_X0 


lim_inf_C_B0 = 0.04 # Cota inferior
lim_sup_C_B0 = 0.13 # Cota superior 
paso_C_B0 = 0.01 # Paso de iteracion

vector_C_B0 = seq(lim_inf_C_B0,lim_sup_C_B0, by = paso_C_B0) # Vector de valores de barrido de la variable manipulada C_X0. Estos son los valores que va a tener la variable manipulada



# ___ Creacion de vector C_Pf, C_B0_j y C_S0_i vacios
...
for (j in vector_C_B0){
  
  C_B0 = j # Variable manipulada 1
  
  for (i in vector_C_S0) {
    
    C_S0 = i #g/L # Variable manipulada 2
    ...
    C_Pf_ji = res.C_P[n_t]# Este es el valor de C_Pf en el cada paso de iteracion 
    
    C_Pf = c(C_Pf,C_Pf_ji) # Aca le agrego al vector C_Pf el nuevo valor de C_Pf_i obtenido 
...
resultados <- data.frame(C_B0_j, C_S0_i, C_Pf)

fig = plot_ly(resultados, x = C_B0_j, y = C_S0_i, z = C_Pf,
                    color =  C_S0_i)

fig <- fig %>% layout(title = 'C_Pf en función de C_B0 y C_S0',
                      
                      scene = list(xaxis = list(title = 'C_B0 [g/L]'),
                                   
                                   yaxis = list(title = 'C_S0 [g/L]'),
                                   
                                   zaxis = list(title = 'C_Pf [g/L]')))
